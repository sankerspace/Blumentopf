...
void loop(void)
{
  ...
  else                                      // no message arrived. Checking the schedule
  {
    ...
    {
      if (myNodeList.getNodeType(0))    // first node (index=0) in the list  is a motor node 
      {
        DEBUG_PRINT("Node list id: ");
        DEBUG_PRINTLN(myNodeList.myNodes[0].ID);
        if (myNodeList.isActive(0) == 1)//check if the first node (index=0)in the list is active
